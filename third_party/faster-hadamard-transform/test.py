...
truth_hadamards = [
    torch.tensor(scipy.linalg.hadamard(size), device="cuda", dtype=torch.float32)
    * get_scale(size)
    for size in test_sizes_m
]
    ...
    for elem_c in test_elem_counts:
        ...
        if correctness_check:
            for i in range(runs_per_size):
                ...
                truth_fp16 = truth_hadamard_transform_inplace(
                    a_truth_fp16, truth_hadamards_fp16
                )
                check_correctness(
                    m, elem_c, a, result_fp16, truth_fp16, atol=1e-2
                )  # TODO: NOTE: we are not accurate down to 3 decimal places (atol)
                ...
                truth_bf16 = truth_hadamard_transform_inplace(
                    a_truth_bf16, truth_hadamards_bf16
                )
                check_correctness(
                    m, elem_c, a, result_bf16, truth_bf16, atol=5e-2
                )  # TODO: NOTE: need 5x atol to pass for bf16
        else:
            # run in a row so that warmup is valid
            a_result = (
                a  # we can clobber the result cause we are only interested in timing
            )
            for i in range(runs_per_size):
